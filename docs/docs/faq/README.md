...
通常客户端提示：`无法验证用户名` 或者 `暂时无法连接到身份验证服务器` 又或者 `身份验证服务器宕机`。

![image-20251018004951995](./assets/image-20251018004951995.png)

服务端可能提示：`servers are unavailable`。

![image-20251018005046777](./assets/image-20251018005046777.png)

解决方案：

- ==如果您没有正版账号== ，请在服务器设置关闭正版验证，并采用相关 ==登录插件== 保护您的服务器。（或者采用 ==外置登录== 的解决方案）。

- 如果您使用了正版账号，还是出现如上提示，可以尝试给 ==服务器/游戏客户端== 使用加速器。!!正版受害者.jpg!!

  ::: tip 如何判断是服务端还是客户端连不上正版验证？

  如上图，若服务器有错误提示`服务不可达`的，基本是服务端连不上。
